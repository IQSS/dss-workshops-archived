...
** Batch examples
RCE users come from a variety of backgrounds and different people are more proficient with different software packages. Rather than show you generic examples, or pick an application to showcase, we've prepared a separate page with [[file:examples/RCE_batch_examples.html][examples using different software packages]]. To walk through these examples click the link below corresponding to the software you plan to use on the RCE.
