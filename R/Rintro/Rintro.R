...
## ------------------------------------------------------------------------
## rows 2 and 4
slice(baby.names, 1:3) ## first three rows

#' 
## ------------------------------------------------------------------------
## rows where Name == "jill"
filter(baby.names, Year == 1996 & Name == "jill")

#' 
## ------------------------------------------------------------------------
## rows where Name is either "jack" or "jill"
...
## modify example.df and assign the modified data.frame the name example.df
baby.names <- mutate(baby.names,
                     Country = case_when(Location == "England and Wales" ~ "UK",
                                         Location != "England and Wales" ~ "US"))
glimpse(baby.names)
...
write_csv(baby.names, path = "babyNames.csv")

# write data to an R file
write_rds(baby.names, path = "babyNames.rds")

# write data to a spss file
library(haven)
write_sav(baby.names, path = "babyNames.dta")
...
#' 
#' 2.  Filter `baby.names` to include only names given to at least 3
#'     percent of Girls. Save this to a SPSS data set
#'     named "popularGirlNames.sav")
## ------------------------------------------------------------------------
## write your answer here


#' 
#' 3.  (Bonus) if you look at `unique(baby.names$Sex)` you'll notice that some records indicate Male with `"M"`, while other records use `"Male"`. Correct this by replacing `"M"` with `"Male"`.
#' 
...
#' 
## ------------------------------------------------------------------------
summarize(group_by(baby.names, Country), mean(Name.length), sd(Name.length))

#' 
#' You can group by multiple variables:
#' 
## ------------------------------------------------------------------------
summarize(group_by(baby.names, Country, Sex), mean(Name.length), sd(Name.length))
...
print("This is the summary of baby.names \n")
print(summary(baby.names))
