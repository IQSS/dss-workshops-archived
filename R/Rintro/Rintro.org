...
#+name: GSSinfo
#+begin_src R :results value org table :colnames yes :exports results
  read.csv("dataSets/gssInfo.csv")
...
1. Read the Excel data set in dataSets/gss.xlsx and assign the result to an R object named GSS.xl
...
6. Check if the dimensions of GSS.xl are equal to the values you stored in gss.dim
...
+                   )
> str(gss_1.4)
'data.frame':	1419 obs. of  4 variables:
 $ age     : num  69 27 19 21 19 87 42 19 78 70 ...
 $ educ    : num  12 10 11 9 11 8 11 11 7 9 ...
 $ emailhrs: num  -1 -1 0 -1 50 -1 3 -1 -1 -1 ...
 $ hrs1    : num  -1 60 32 20 -1 -1 -1 -1 -1 22 ...
...
'data.frame':	2 obs. of  2 variables:
 $ sex: Factor w/ 2 levels "Male","Female": 2 1
 $ age: num  99 99
> ## the $ operator can be used to extract a single column
> str(gss_gt90$age)
 num [1:2] 99 99
...
2. Generate a variable named "dual.earn" equal to 1 if both wkftwife = 1 and wkfthusb = 1, and zero otherwise. How many dual earners are there?
3. Create a subset of the data containing only rows where "usecomp" = "Yes"
4. Examine the data.frame created in step 2, and answer the following questions:
...
2) [@2] Read the SPSS data set in dataSets/gss.sav and assign the result to an R data object named GSS.sav
#+BEGIN_SRC R :results silent
  GSS.sav <- import("dataSets/gss.sav")
#+END_SRC

3) [@3] Make sure that the data loaded in step 2 is a data.frame
#+BEGIN_SRC R
  class(GSS.sav)
#+END_SRC
...
4) [@4] Display the dimensions of the GSS.sav. 
#+BEGIN_SRC R
  dim(GSS.sav)
  nrow(GSS.sav)
  ncol(GSS.sav)
...
5) [@5] Save the dimensions of GSS.sav to an object named gss.dim
...
7) [@7] Check if the dimensions of GSS.xl are equal to the values you stored in gss.dim
#+BEGIN_SRC R
  dim(GSS.xl)
  gss.dim
  gss.dim == dim(GSS.xl)
...
1) [@1] Generate the following variables:
   - "rich" equal to 0 if rincdol is less than 100000, and 1 otherwise
   - "sinc" equal to incomdol - rincdol
#+BEGIN_SRC R
  gss <- transform(gss,
...
2) [@2] Generate a variable named "dual.earn" equal to 1 if both wkftwife = 1 and wkfthusb = 1, and zero otherwise.
#+BEGIN_SRC R
...
3) [@3] Create a subset of the data containing only rows where "usecomp" = "Yes".
#+BEGIN_SRC R
...
4) [@4] How many computer and non computer users?
#+BEGIN_SRC R
...
4) [@4] Create a scatter plot with educ on the x-axis and incomdol on the y-axis
#+BEGIN_SRC R :results output graphics :file images/ex3.png :height 300
...
#+BEGIN_SRC R :results output graphics :file images/ex3_1.png :height 300
