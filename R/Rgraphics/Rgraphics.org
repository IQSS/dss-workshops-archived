...
#+BEGIN_SRC R :results silent
  ggplot(dat, aes(x = CPI, y = HDI)) +
    geom_point()
#+END_SRC
2. [@2] Overlay a smoothing line on top of the scatter plot using the /lm/ method. Hint: see =?stat_smooth=.
#+BEGIN_SRC R :results silent
  ggplot(dat, aes(x = CPI, y = HDI)) +
    geom_point() +
    geom_smooth(method = "lm")
#+END_SRC
3. [@3] Overlay a smoothing line on top of the scatter plot using the default method.
#+BEGIN_SRC R :results silent
  ggplot(dat, aes(x = CPI, y = HDI)) +
    geom_point() +
    geom_smooth()
#+END_SRC
4. [@4] BONUS (optional): Overlay a smoothing line on top of the scatter plot using the default /loess/ method, but make it less smooth. Hint: see =?loess=.
#+BEGIN_SRC R :results silent
  ggplot(dat, aes(x = CPI, y = HDI)) +
    geom_point() +
    geom_smooth(span = .4)
...
This more or less gets the information across, but the labels overlap in a most unpleasing fashion. This part is tricky. Unfortunately there is no simple way to automatically adjust the labels so they dont't overlap. We can use some heristics to improve things a bit, but We'll have to do some final positioning more or less manually.

#+BEGIN_SRC R :results output graphics :file images/econScatter5.png :width 800 :height 600
  dat$hdi.l <- dat$HDI
  dat[dat$Country == "Sudan", "hdi.l"] <- dat[dat$Country == "Sudan", "hdi.l"] + 0.01
  dat[dat$Country == "United States", "hdi.l"] <- dat[dat$Country == "United States", "hdi.l"] + 0.02
  dat[dat$Country == "Japan", "hdi.l"] <- dat[dat$Country == "Japan", "hdi.l"] - 0.02


  (pc4 <- pc3 + geom_text(aes(y = hdi.l,
                              label = Country,
                              hjust = (1 - CPI/max(CPI)),
                              vjust = HDI/max(HDI)),
                          color = "gray10",
...
#+RESULTS:
[[file:images/econScatter5.png]]

We could use this method to adjust more points, or we could do some post-processing of the image in something like [[http://inkscape/org][Inkscape]], but for now I'll call it good enough. Some of the points are obscured by the labels (unlike in the original), but the tradoff is that our labels are actually closer to the point in space that they correspond to. In my opinion moving the labels farther away from the points (as was done in the original) makes it harder to grasp the information being conveyed, so I'll leave it there.
...
#+BEGIN_SRC R :results output graphics :file images/econScatter8.png :width 800 :height 600
  library(grid) # for the 'unit' function
  (pc6 <- pc5 +
    theme_minimal() + # start with a minimal theme and add what we need
    theme(text = element_text(color = "gray20"),
          legend.position = c(0.0, 1.05), # position the legend in the upper left 
          ...
          plot.margin = unit(c(15, 2, 15, 4), "mm"), # make some space at top and bottom
          plot.title = element_text(face = "bold", hjust = -.07, vjust = 5) # bad hack to get the title in the right spot.
          ))
